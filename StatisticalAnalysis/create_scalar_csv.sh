mkdir -p "data"
for data in "results" "results_no_transient" "single-results" "single-results_no_transient"
do
  mkdir -p data/data_${data}
  cd ../${data}
  for config in "First" "Second" "Third"
  do
    for n in 2 3 4 5
    do
      mkdir -p ../StatisticalAnalysis/data/data_${data}/${config}-n${n}
      ...
      scavetool x -f 'module("*.CS") AND (name("timeInsideSystem:mean"))' -o ../StatisticalAnalysis/data/data_${data}/${config}-n${n}/timeInsideSystemMean.csv -F CSV-S ${config}-n${n}-*.sca
      scavetool x -f 'module("*.CS") AND (name("timeInsideSystem:max"))' -o ../StatisticalAnalysis/data/data_${data}/${config}-n${n}/timeInsideSystemMax.csv -F CSV-S ${config}-n${n}-*.sca
    done
  done
done